...
public class ProteinCollectionSerializerExtensionsTests
{
    ...
    public async Task ProteinCollectionSerializerExtensions_ReadUriAsync_Test()
    {
        var serializer = new ProjectSummaryJsonDeserializer();
        var proteins = await serializer.ReadUriAsync(new(ProjectSummaryUrl.Json));
        Assert.AreNotEqual(0, proteins.Count);
    }
